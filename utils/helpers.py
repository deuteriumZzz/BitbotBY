...
def generate_signals(df: pd.DataFrame) -> Dict[str, bool]:
    ...
    signals = {
        ...
        'macd_bearish': latest['macd'] < latest['macd_signal'] if pd.notna(latest['macd']) else False,
        'price_above_sma20': latest['close'] > latest['sma_20'] if pd.notna(latest['sma_20']) else False,
        'price_below_sma20': latest['close'] < latest['sma_20'] if pd.notna(latest['sma_20']) else False,
        'golden_cross': latest['sma_20'] > latest['sma_50'] if pd.notna(latest['sma_50']) else False,
        'death_cross': latest['sma_20'] < latest['sma_50'] if pd.notna(latest['sma_50']) else False,
        'stoch_overbought': latest['stoch_k'] > 80 if pd.notna(latest['stoch_k']) else False,
        'stoch_oversold': latest['stoch_k'] < 20 if pd.notna(latest['stoch_k']) else False
    }
