...
def add_news_sentiment(df, news_list):
    ...
    df["sentiment"] = (
        avg_sentiment  # Добавляем как константу для простоты; можно усреднить по времени
    )
    return df
...
def main(auto=False):
    strategy_name = (
        "scalping" if not auto else None
    )  # Если auto, обучаем для всех стратегий
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--auto", action="store_true", help="Auto-train for all strategies"
    )
    parser.add_argument(
        "strategy",
        nargs="?",
