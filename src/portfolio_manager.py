...
class PortfolioManager:
    ...
    async def update_portfolio(self, symbol: str, action: str, quantity: float, price: float) -> bool:
    ...
    async def rebalance_portfolio(self, target_allocations: Dict[str, float], current_prices: Dict[str, float]):
        ...
        for symbol, target_allocation in target_allocations.items():
            ...
            current_value = await self.get_position_size(symbol) * current_prices.get(symbol, 0)
            
            if current_value < target_value:
                # Need to buy
                buy_value = target_value - current_value
                quantity = buy_value / current_prices[symbol]
                await self.update_portfolio(symbol, 'buy', quantity, current_prices[symbol])
            
            elif current_value > target_value:
                # Need to sell
                sell_value = current_value - target_value
                quantity = sell_value / current_prices[symbol]
                await self.update_portfolio(symbol, 'sell', quantity, current_prices[symbol])
