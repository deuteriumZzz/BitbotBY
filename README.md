...
- 5% за день → все позиции немедленно закрываются рыночными ордерами, торговля останавливается до следующего UTC-дня
- В paper режиме: предупреждение в Telegram, торговля продолжается

**Уровень 3 — Аварийные стоп-краны** (black swan)
- **Hard drawdown halt** — просадка equity от пика ≥ 20% подтверждённая N циклов подряд (защита от flash crash) → закрыть всё, пауза `DRAWDOWN_HALT_HOURS` часов. Количество циклов (flash-защита) меняется через Telegram: ⚖️ Риск-профиль → 🛡 Flash-защита ±
- **ATR spike circuit breaker** — ATR в 3× выше среднего за 20 свечей → позиции закрыть, новые не открывать 1 час
- Оба триггера сначала **закрывают все позиции по рынку** (отменяют биржевые SL/TP, создают market-ордера), затем уходят в паузу. В paper режиме — только уведомление
...
A: Бот немедленно: (1) отменяет биржевые SL/TP ордера всех позиций, (2) закрывает каждую позицию рыночным ордером, (3) уходит в паузу на `DRAWDOWN_HALT_HOURS` часов. В paper режиме — только уведомление в Telegram, торговля продолжается. Количество циклов подтверждения перед срабатыванием настраивается через ⚖️ Риск-профиль → 🛡 Flash-защита.
