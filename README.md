...
При `AI_PROVIDER=auto` бот пробует провайдеры в порядке: Claude → OpenAI → DeepSeek → Groq. Если основной провайдер вернул ошибку квоты (HTTP 429/402) — автоматически переключается на следующий.
...
| `/lev` | Показать текущий режим управления плечом |
...
|-------|---------|-------------|
| `fixed` | `/lev fixed` | Всегда использует `LEVERAGE` из `.env` (например 3x) |
| `volatility` | `/lev volatility` | ATR-таргетинг: высокая волатильность → меньше плечо, спокойный рынок → больше. Целевой риск на ATR-движение = 1% (настраивается) |
| `full` | `/lev full` | То же, что volatility, + понижает плечо в даунтренде (×0.5) и при просадке баланса (до ×0.3) |

По умолчанию: `volatility`. Изменить целевой риск: `/lev target 0.02` (= 2% на ATR-движение).

```
Пример:
BTC ATR = 500 USDT, цена = 50 000 → ATR% = 1%
