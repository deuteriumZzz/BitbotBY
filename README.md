...
Гибридный крипто-трейдинг бот на Python: **Claude / DeepSeek / OpenAI / Groq** + SAC нейросеть + рыночные сигналы реального времени + кванты (CVaR, Almgren-Chriss, Kelly) + 9 стратегий.  
...
| `/lev` | Показать текущий режим плеча, риск и порог просадки |
| `/lev fixed` | Фиксированное плечо из `LEVERAGE` в `.env` |
| `/lev volatility` | ATR-таргетинг: бот сам выбирает плечо под волатильность монеты |
| `/lev full` | ATR + режим рынка (тренд/боковик/даун) + защита при просадке |
| `/lev target 0.02` | Целевой риск на ATR-движение (0.5%–10%, по умолчанию 1%) |
| `/lev drawdown 0.15` | Порог просадки для режима `full`: при -15% баланса плечо снижается до 30% |
| `/provider` | Показать текущий AI-провайдер |
| `/provider groq` | Переключить AI-провайдер (`auto\|anthropic\|openai\|deepseek\|groq`) |
...
**594 unit-теста** — индикаторы, стратегии, риск-менеджмент, CVaR, Kelly, Almgren-Chriss, корреляция, SAC-инференс, dynamic leverage, partial TP, drawdown scaling, liquidity filter, dynamic exits, position monitor, online learner, runtime config, e2e торговый цикл.  
Coverage: **93%**.
...
A: Не должен терять — настройки хранятся в Redis. Если Redis тоже перезапустился (например `make down && make up`), данные Redis сбрасываются и бот стартует с `.env`. Используйте кнопку "🔄 Сброс настроек" в Telegram только если хотите вернуться к значениям `.env`.
