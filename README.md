...
> **Совет для старта (бесплатно):** добавь оба бесплатных ключа — Groq + Gemini. Groq отработает первые ~13 запросов, Gemini покроет остаток дня (1500 запросов/сутки). Для реальной торговли дополнительно пополни DeepSeek (~$5/мес).

**Сравнение провайдеров:**

| Провайдер | Скорость | Стоимость | Качество | Бесплатный тир |
|-----------|----------|-----------|----------|----------------|
| Groq (Llama 3.3 70B) | ⚡ Очень быстро | бесплатно | Хорошее | ✅ 100k токенов/день |
| Gemini Flash | ⚡ Очень быстро | бесплатно | Хорошее | ✅ 1500 запросов/день |
| DeepSeek | Быстро | ~$0.002 | Хорошее | ❌ |
| Claude Sonnet | Умеренно | ~$0.01 | Отличное | ❌ |
| OpenAI GPT-4o-mini | Умеренно | ~$0.003 | Хорошее | ❌ |

При `AI_PROVIDER=auto` бот пробует провайдеры в порядке: **Claude → OpenAI → DeepSeek → Groq → Gemini**. Fallback срабатывает при любой ошибке (429, 402, 500, таймаут, сеть) — кроме HTTP 400 (неверный запрос). Переключить провайдера без перезапуска: `/provider groq` в Telegram.
